...
<script>
    ...
    const fetchMusic = async (query) => {
        if (!query) {
            // Limpa a grade e não exibe nada se a busca estiver vazia
            musicGrid.innerHTML = '';
            noResults.style.display = 'none';
            return;
        }

        // Exibe uma mensagem de carregamento ou esconde resultados antigos
        musicGrid.innerHTML = '';
        noResults.style.display = 'none';

        // Endpoint da API do iTunes
        ...
        try {
            ...
            noResults.textContent = 'Ocorreu um erro ao carregar os dados. Por favor, tente novamente mais tarde.';
            noResults.style.display = 'block';
        }
    };

    // Função para renderizar os cards de música
    const renderMusicCards = (songs) => {
        songs.forEach(song => {
            const card = document.createElement('div');
            card.className = 'music-card';

            card.innerHTML = `
                <img src="${song.artworkUrl100.replace('100x100bb', '400x400bb')}" alt="${song.trackName} - ${song.artistName}">
